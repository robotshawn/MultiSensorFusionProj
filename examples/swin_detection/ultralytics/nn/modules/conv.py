...
__all__ = ('Conv', 'Conv2', 'LightConv', 'DWConv', 'DWConvTranspose2d', 'ConvTranspose', 'Focus', 'GhostConv',
           'ChannelAttention', 'SpatialAttention', 'CBAM', 'Concat', 'RepConv','DoubleAttention','GAM_Attention')
...
class GSConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        ...
        #self.cv3 = Conv1(c_, c_, 5, 1, None, c_, act)
    def forward(self, x):
        x1 = self.cv1(x)
        x2 = torch.cat((x1, self.cv2(x1)), 1)
        #x2 = torch.cat((x1, self.cv3(x1)), 1)  #去掉DW卷积
        # shuffle
