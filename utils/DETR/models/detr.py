...
class DETR(nn.Module):
    ...
    def forward(self, feature):
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, C, L**0.5, L**0.5))  
        trunc_normal_(self.pos_embed, std=0.02)
        pos = self.pos_embed.expand(B, -1, -1, -1)

        mask = torch.zeros((B, L**0.5, L**0.5), dtype=torch.bool)

        # mask: Tensor [bs,19,26]
        assert mask is not None

        # 数据输入transformer进行前向传播
        # feature: [bs,256,19,26]
        # mask: False的区域是不需要进行注意力计算的
        # self.query_embed.weight  类似于传统目标检测里面的anchor 这里设置了100个
        # pos  位置编码  [bs, 256, 19, 26]
        # hs: [6, bs, 100, 256]
        hs = self.transformer(feature, mask, self.query_embed.weight, pos)[0]
